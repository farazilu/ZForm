...
if (! $is_ajax || (! $list_form_loaded && ! $page)) {
    Redback::instance()->scripts("assets/core/js/list.js");
    if ($addNewButton) {
        ?>
<div class="container">
	<div class="ui-accordion-header ui-helper-reset ui-state-default  ui-corner-top sixteen columns alpha omega">
		<?php echo Html::anchor($controller. '/edit' ,'<span class="ui-button-text"> '.__('add_new').'</span>', array('class'=>"$tabClass ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only", 'title'=>'Add New' ));?>
	</div>
	<?php } ?>
